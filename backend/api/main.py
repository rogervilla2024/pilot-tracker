"""
Pilot Tracker - Backend API

FastAPI application for serving crash game statistics.
This module provides REST API endpoints for accessing Pilot game data
...
ALLOWED_ORIGINS: List[str] = [
    "http://localhost:3000",
    "http://localhost:3001",
    os.getenv("FRONTEND_URL", "https://pilottracker.com"),
    os.getenv("SECONDARY_DOMAIN", "https://www.pilottracker.com"),
]
...
app = FastAPI(
    title="Pilot Tracker API",
    description="Real-time statistics API for Gamzix Pilot crash game",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup() -> None:
    ...
    """
    logger.info("Starting Pilot Tracker API...")
    ...
    try:
        async with db_manager.connect() as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_round_id ON pilot_rounds(round_id)"
            )
...
async def get_rounds(
    ...
    try:
        # Get total count
        cursor = await db.execute("SELECT COUNT(*) FROM pilot_rounds")
...
async def get_summary(db: aiosqlite.Connection = Depends(get_db)) -> SummaryStats:
    ...
    try:
        ...
        # Calculate median
        cursor = await db.execute("""
            SELECT crash_multiplier FROM pilot_rounds
            ORDER BY crash_multiplier
            LIMIT 1 OFFSET (SELECT COUNT(*) FROM pilot_rounds) / 2
        """)
...
async def get_distribution(
    ...
    try:
        ...
        for name, condition in DISTRIBUTION_BUCKETS:
            cursor = await db.execute(
                f"SELECT COUNT(*) FROM pilot_rounds WHERE crash_multiplier {condition}"
            )
...
async def health() -> HealthResponse:
    ...
    try:
        async with db_manager.connect() as db:
            ...
            cursor = await db.execute(
                "SELECT MAX(created_at) FROM pilot_rounds"
            )
...
crash_stats_router = create_crash_game_router(db_manager, "pilot")
...
if __name__ == "__main__":
    ...
    logger.info("Starting Pilot Tracker API server...")
